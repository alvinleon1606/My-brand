document.addEventListener('DOMContentLoaded', async function() {
    ...
    const getSkillDetails = async(skillId) => {
        try {
            const response = await fetch(`https://leonx-ldu1-vvuz.onrender.com/skills/${skillId}`);
            if (!response.ok) {
    ...
    async function updateSkill(updatedSkill) {
        try {
            const response = await fetch(`https://leonx-ldu1-vvuz.onrender.com/skills/edit/${skillId}`, {
                method: 'PUT',
                headers: {
