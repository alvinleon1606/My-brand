document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const newPassword = document.getElementById('new1-password').value.trim();
        ...
        if (!passwordRegex.test(newPassword)) {
            passwordError.style.color = 'red';
            newPasswordError.innerHTML = 'Please enter a valid password with at least 5 characters, containing at least one digit, one uppercase letter, one lowercase letter, and one special character. No spaces allowed.';
            document.getElementById('new1-password').style.borderColor = 'red';
        } else {
            document.getElementById('new1-password').style.borderColor = 'green';
        }
        ...
        if (namesError.innerHTML === '' && emailError.innerHTML === '' && passwordError.innerHTML === '' && comfirmPasswordError.innerHTML === '') {
            // Submit the form
        }
    });
    ...
    // New password
    document.getElementById('new1-password').addEventListener('input', function() {
        const password = this.value.trim();
        const passwordRegex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{5,15}$/;
